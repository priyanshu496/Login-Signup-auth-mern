...
const Login = () => {
  ...
  const onSubmitHandler = async (e) =>{
    try{
      ...
      if(state === 'Sign Up'){
        const {data} = await axios.post(backendUrl + '/api/auth/register', {name, userId, email, password});
        if(data.success){
          setIsLoggedIn(true);
          getUserData(email);
          navigate('/');
        ...
        const {data} = await axios.post(backendUrl + '/api/auth/login', {userId, email, password});
        if(data.success){
          setIsLoggedIn(true);
          getUserData(email);
          navigate('/');
        ...
        <div className='bg-slate-900 p-10 rounded-lg shadow-lg w-full sm:w-96 text-indigo-300 text-sm'>
          ...
          <form onSubmit={onSubmitHandler}>
            ...
            <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
              <img src={assets.person_icon} alt="" />
              <input onChange={e => setUserId(e.target.value) } value ={userId} className='bg-transparent outline-none text-white' type="userId" placeholder='User Id' required/>
            </div>
            <div className='mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]'>
